...
/**
 * Android/JVM actual using java.math.BigInteger.
 *
 * Replaces the bignum-based commonMain implementation which crashes on Android
 * due to a bug in BigInteger32Arithmetic.shiftLeft inside the division algorithm
 * (triggered by every .mod() call in the field arithmetic).
 *
 * java.math.BigInteger is:
 *  - crash-free on all Android versions
 *  - significantly faster (native implementation)
 *  - has a built-in modInverse(m) that avoids the problematic code path
 */
internal actual object Ed25519Icarus {
    ...
    private fun fromLittleEndian(le: ByteArray): BigInteger =
        BigInteger(1, le.reversedArray())   // signum=1 → always positive

    /** Big-endian java.math.BigInteger → 32-byte little-endian byte array */
    private fun toLittleEndian32(n: BigInteger): ByteArray {
        val dst = ByteArray(32)
        // java.math.BigInteger.toByteArray() is big-endian, may have a leading 0x00 sign byte
        val be  = n.toByteArray()
        val len = minOf(be.size, 32)
        for (i in 0 until len) {
            dst[i] = be[be.size - 1 - i]   // reverse from end: naturally skips any sign byte
        }
        return dst
    }

    /** Modular inverse via java.math.BigInteger.modInverse (extended Euclidean, no crash) */
